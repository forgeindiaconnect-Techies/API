...
async def get_dashboard(current_user=Depends(get_current_user)):
    ...
    if hasattr(db.messages._collection, "aggregate"):
        try:
            ...
            aggregated_results = await cursor.to_list(None)

            # Today's count: count_documents is fast and indexed
            requests_today = await db.messages.count_documents({
                "user_id": user_id,
                "role": "user",
                "$or": [
            ...
            seven_days_date = seven_days_ago.date()
            for row in aggregated_results:
                if not row.get("_id"):
                    continue
                try:
                    row_date = datetime.strptime(row["_id"], "%Y-%m-%d").date()
                except Exception:
                    continue

                if row_date in daily_stats:
                    daily_stats[row_date]["requests"] = row.get("requests", 0)
                    daily_stats[row_date]["tokens"] = row.get("tokens", 0)

                if row_date >= seven_days_date:
                    requests_this_week += row.get("requests", 0)
                    tokens_this_week += row.get("tokens", 0)

            use_fallback = False
        except Exception as ae:
            import logging
            logging.getLogger(__name__).warning(f"14-day stats aggregation failed: {ae}")
...
async def get_usage(
    ...
    if hasattr(db.messages._collection, "aggregate"):
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"{days}-day usage stats aggregation failed: {ae}")
